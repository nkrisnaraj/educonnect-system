...
const studentChatBox = ({  }) => {
  ...
  const loadMessages = async (token) => {
    const response = await api.get(`students/messages/${selectedChat}/`, {
      headers: {
    ...
    const transformed = (Array.isArray(response.data) ? response.data : []).map(
      (msg) => ({
        sender: msg.sender.id === user?.id ? 'student' : msg.sender.role, // Fix this logic
  ...
  try {
    const response = await api.post(`students/messages/${selectedChat}/send/`, messagePayload, {
      headers: {
    ...
    const savedMessage = {
      sender: 'student', // Always 'student' for sent messages
     ...
     <div className="w-full  border border-gray-200 rounded-xl overflow-hidden flex flex-col h-[500px]">
      ...
      <div className="bg-white p-3 flex-1 overflow-y-auto flex flex-col space-y-3">
        {Array.isArray(messages) && messages.map((msg, index) => (
          ...
          >
            <div
              className={`relative rounded-2xl p-3 max-w-[75%] shadow-sm ${
                msg.sender === 'student' 
